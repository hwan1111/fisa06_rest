...
def render_pro_map(disp_df):
    ...
    for _, rest in unique_restaurants.iterrows():
        ...
        all_reviews = disp_df[disp_df['restaurant_id'] == rest['restaurant_id']].copy()
        
        # [수정] 데이터 타입 강제 변환 및 안전한 평균 계산
        if not all_reviews.empty and 'rating' in all_reviews.columns:
            all_reviews['rating'] = pd.to_numeric(all_reviews['rating'], errors='coerce')
            # dropna를 사용하여 NaN 값 제외 후 평균 계산
            avg_rating = all_reviews['rating'].dropna().mean()
            
            # 모든 리뷰가 NaN이어서 평균이 NaN인 경우 0.0 등으로 처리
